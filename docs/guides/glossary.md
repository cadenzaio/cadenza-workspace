...
- **Actor:** a durable keyed behavior identity with serialized task execution,
  assignment epochs, owner residency, hydration, and strict write-through state.
- **Affect:** a consequential change an identity can make.
- **Authority:** the current legitimate basis for affect. Serialized definition
  authority and durable environment authority are distinct.
- **Cell:** the trusted local runtime substrate that owns containment, process
  custody, routing, peer transport, capabilities, and evidence custody.
- **Chamber:** the contained runtime that admits immutable images, supervises a
  language adapter, materializes primitives, and executes them.
- **Conclusion:** deterministic composition of terminal graph changes against
  the run-entry context, or an explicit provenance-bearing conflict.
- **Context:** business data passed through a graph. Runtime identity and
  credentials are not injected into it.
- **Custody:** accountable possession of an artifact, process, credential, or
